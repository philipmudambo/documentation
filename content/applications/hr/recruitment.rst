...
scheduling a phone call, conducting an interview, and sending a job offer, just to name a few. This
...
Before creating a job position to be recruited for, the various settings for the *Recruitment* app
should first be configured. To view and edit the settings, navigate to :menuselection:`Recruitment
app --> Settings`. After any changes are made, click the :guilabel:`Save` button in the top-left
corner to save all the changes.

Job posting
-----------

The *Job Posting* section of the *Recruitment* app settings has only one selection to make. If job
positions are to be posted to the company's website, enable the :guilabel:`Online Posting` option.

This option only works if the *Website* application is also installed.
...
Odoo is capable of having a survey, or exam, sent to an applicant to better determine their skill
set. This aids the recruitment team and helps them determine which candidates should be moved
through the recruitment pipeline, and which should no longer be considered.

Enable the :guilabel:`Send Interview Survey` option to be able to send surveys to applicants.

.. note::
   The *Surveys* application must be installed to use this feature.

Send SMS
~~~~~~~~

It is possible to send text messages to applicants directly through the *Payroll* app. To do so,
enable the :guilabel:`Send SMS` option. This option requrtes credits to use. For more information,
refer to the :doc:`SMS pricing and FAQs
...
When applicants submit an application, one of the default required fields is a resumé, or CV. All
resumés are stored in the *Documents* application, and are accessible on the applicant's card.

A resumé has the option to appear on the applicant's card, instead of having a document link appear
in the chatter, where it then needs to be expanded or downloaded to view it.
...
If the window or tab is not wide enough, the resumé does not appear. The screen must be wide enough
to accommodate the resumé. The full-screen view typically accommodates this view.
...
When an application is submitted, it is possible to have Odoo extract the applicant's name, phone
number, and email address directly from the resumé. To do so, enable the :guilabel:`CV Digitization
(OCR)` option.
...
Beneath these options are two additional links. Click the :guilabel:`→ Buy credits` button to
purchase credits for CV digitization. Click the :guilabel:`→ View My Services` to view a list of all
...
:doc:`In-app purchase (IAP) </applications/general/in_app_purchase>` documentation.
...
The last column, :guilabel:`Contract Signed`, is folded by default. Folded column appear gray, and
...
To create a new stage, click on :guilabel:`+ Stage` and a new column appears. Enter the title for
the new stage in the :guilabel:`Stage title` field, then click :guilabel:`Add`. The new column
appears, and another new stage is available to create. If no new stages are needed, click anywhere
on the screen to exit the new stage creation.
...
a menu appears. Then click on the :guilabel:`Edit` option. An :guilabel:`Edit: (Stage)` form
appears. Make any desired modifications to the form, then click :guilabel:`Save & Close` when done.
...
to reveal a drop-down menu, then click :guilabel:`Delete`. A :guilabel:`Confirmation` pop-up warning
appears, asking :guilabel:`Are you sure you want to delete this column?` Click :guilabel:`Delete`
...
as the applicant's email address.

.. image:: recruitment/full-composer.png
   :align: center
   :alt: Send an email from the chatter.

Click the :guilabel:`⤢ (double arrow)` full composer icon in the bottom right corner of the
:guilabel:`Send Message` tab in the chatter. A :guilabel:`Compose Email` pop-up window loads, with
the :guilabel:`Recipients` and :guilabel:`Subject` pre-populated. The applicant's email address is
entered in the :guilabel:`Recipients` line, and the :guilabel:`Subject` is `(Job Position)`. The
...
attachment, click the :guilabel:`X (Delete)` icon to the right of the attachment.
...
.. important::
   ...
   if modifications are made to create a customized recruitment pipeline flow, it varies from the
...
If a resumé was attached to the online application, the resumé appears in the attachments section of
the chatter, and is also stored in the *Documents* application. To find the recruitment documents
stored in the *Documents* application, navigate to the main :menuselection:`Documents` dashboard,
and click the folder labeled :guilabel:`Recruitment` on the left-hand side. All recruitment
...
- :guilabel:`Doesn't fit the job requirements`: uses the :guilabel:`Recruitment: Refuse` email
  template.
- :guilabel:`Refused by Applicant: don't like job`: uses the :guilabel:`Recruitment: Not interested
  anymore` email template.
- :guilabel:`Refused by Applicant: better offer`: uses the :guilabel:`Recruitment: Not interested
  anymore` email template.
- :guilabel:`Language issues`: uses the :guilabel:`Recruitment: Refuse` email template.
- :guilabel:`Role already fulfilled`: uses the :guilabel:`Recruitment: Refuse` email template.
- :guilabel:`Duplicate`: uses the :guilabel:`Recruitment: Refuse` email template.
- :guilabel:`Spam`: uses the :guilabel:`Recruitment: Refuse` email template.
- :guilabel:`Refused by Applicant: salary`: uses the :guilabel:`Recruitment: Not interested anymore`
  email template.
...
The applicant's email automatically populated the :guilabel:`Send Email` field; additional emails
cannot be added. If an email should **not** be sent to the applicant, uncheck the :guilabel:`Send
Email` checkbox.
...
new blank applicant card appears. Click either the :guilabel:`🗑️ (delete)` icon or anywhere on the
screen to close the card.
...
To change the displayed date range for the calendar, either use the :guilabel:`← (Left arrow)`,
:guilabel:`→ (Right arrow)`, or :guilabel:`Today` buttons above the calendar, or click on a date in
the calendar on the right side of the displayed calendar.
...
To send the meeting via email, click the :guilabel:`✉️ Email` button next to the list of attendees.
...
The followers of the document (job application), as well as the user who created the meeting are
added as :guilabel:`Recipients` by default. Add the applicant's email address to the list to send
the email to the applicant as well. Make any other desired changes to the email. If an attachment is
needed, click the :guilabel:`Attachments` button, navigate to the file, then click :guilabel:`Open`.
Once the email is ready to be sent, click :guilabel:`Send`.
...
attendees. A :guilabel:`Send SMS Text Message` pop-up appears.
