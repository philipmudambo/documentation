...
The liquidity product direct sale is used, for example, in the cases where sales are made by the
order of a third-party.

For these cases, the seller and the proprietary company of the goods, will be able to register their
corresponding sales and purchase.

This feature is included when :ref:`installing <general/install>` the *Argentinian EDI* module
(`l10n_ar_edi`).
...
This feature includes the addition of the following document types: 23, 24, 25, 26, 27, 28, 33, 43,
45, 46, 48, 58, 60, 61, 150, 151, 157, 158, 161, 162, 164, 166, 167, 171, 172, 180, 182, 186, 188,
332.

It is important to note that by default these document types are *inactive*. For the workflow
to work, it is necessary to *activate* these documents.

To do so, navigate to :menuselection:`Accounting --> Configuration --> Document Types` and remove
any active filters. Then find the documents and activate them by toggling the option
:guilabel:`Active`.
...
Product*. This journal needs to have the sync with AFIP active as this electronic document will be
sent to the AFIP.
...
A sales journal will be needed to register the invoice when the owner of a product sells its good to
the third party that then will sell this. This journal will not have any sync with AFIP as the
invoice will not be electronic.
